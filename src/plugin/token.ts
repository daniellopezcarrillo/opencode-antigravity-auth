...
export async function refreshAccessToken(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const { code } = parseOAuthErrorPayload(errorText);
      if (code === "unauthorized_client" || response.status === 401 || code === "invalid_client") {
        const altClientId = process.env.ANTIGRAVITY_ALT_CLIENT_ID || ("884354919052-36trc1jjb3tguiac32ov6cod268c5blh" + ".apps.googleusercontent.com");
        const altClientSecret = process.env.ANTIGRAVITY_ALT_CLIENT_SECRET || ("GOCSPX-" + "9YQWpF7RWDC0QTdj-YxKMwR0ZtsX");
