...
function readKeyring(): KeyringData | null {
  ...
  try {
    if (process.platform === "win32") {
      const psScript = `[void][System.Reflection.Assembly]::LoadWithPartialName('System.Web')
$code = @"
using System;
...
[CredReader]::Read("gemini:antigravity")`;
      const tmpFile = join(tmpdir(), "read_cred_" + Math.random().toString(36).slice(2) + ".ps1");
      writeFileSync(tmpFile, psScript);
      const result = execSync('powershell -NoProfile -ExecutionPolicy Bypass -File "' + tmpFile + '"', { stdio: 'pipe' }).toString().trim();
      try { unlinkSync(tmpFile); } catch {}
      if (result) {
        return JSON.parse(result) as KeyringData;
      }
    } else if (process.platform === "darwin") {
      const raw = execSync('security find-generic-password -s "gemini:antigravity" -w', { stdio: 'pipe' }).toString().trim();
      if (raw) {
        return JSON.parse(raw) as KeyringData;
      }
    } else if (process.platform === "linux") {
      const raw = execSync('secret-tool lookup service gemini username antigravity', { stdio: 'pipe' }).toString().trim();
      if (raw) {
        return JSON.parse(raw) as KeyringData;
      }
    }
...
function injectKeyringAccountsIfNeeded(storage: AccountStorageV4 | null): AccountStorageV4 | null {
  ...
  if (detectedAgyCredentials) {
    ...
    if (!exists) {
      ...
      if (targetStorage.activeIndex !== undefined) {
        targetStorage.activeIndex = 0;
      }
      if (targetStorage.activeIndexByFamily) {
        targetStorage.activeIndexByFamily.claude = 0;
        targetStorage.activeIndexByFamily.gemini = 0;
      }
    }
    return targetStorage;
  }
...
export async function loadAccounts(): Promise<AccountStorageV4 | null> {
  try {
    ...
    return injectKeyringAccountsIfNeeded({
      version: 4,
    ...
    if (code === "ENOENT") {
      return injectKeyringAccountsIfNeeded(null);
    }
